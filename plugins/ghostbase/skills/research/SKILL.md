...
Use the `ai-cole` subagent when the user needs:
- strategic advice
- positioning
- messaging judgment
...
When delegating to `ai-cole`, include concise context only when it sharpens the coaching question.

3. Combine both tools only when the task needs both facts and strategy.
Typical pattern:
- `search_knowledge_base` first for evidence
- `ai-cole` second for interpretation, prioritization, critique, or framing

Rules:
- Do not use `ai-cole` as a replacement for factual grounding.
- Do not use `ai-cole` as the first pass for raw topic ideation when `search_knowledge_base` can surface better grounded options.
